...
export const SECRET_PATTERNS: Array<[string, RegExp, string]> = [
  ...
  ['AWS Secret Key', /(?:aws_secret_access_key|AWS_SECRET_ACCESS_KEY)\s*[:=]\s*["']?[A-Za-z0-9/+=]{40}/, 'AWS secret access key'],

  // GitHub
  ['GitHub Token', /gh[ps]_[A-Za-z0-9_]{36,}/, 'GitHub personal access token'],
  ['GitHub Fine-Grained', /github_pat_[A-Za-z0-9_]{82,}/, 'GitHub fine-grained personal access token'],

  // Generic API keys
  ['Generic API Key', /(?:api[_-]?key|apikey)\s*[:=]\s*["'][A-Za-z0-9_-]{20,}["']/, 'Generic API key'],
  ['Generic Secret', /(?:secret|password|passwd|pwd)\s*[:=]\s*["'][^"']{8,}["']/, 'Generic secret or password'],
...
export const DANGEROUS_SQL_PATTERNS: Array<[string, RegExp, string]> = [
  ...
  ['DROP COLUMN without IF EXISTS', /DROP\s+COLUMN\s+(?!IF\s+EXISTS)/i, 'Use DROP COLUMN IF EXISTS'],
  ['TRUNCATE TABLE', /TRUNCATE\s+TABLE/i, 'TRUNCATE deletes all data without logging'],
  ['DELETE without WHERE', /DELETE\s+FROM\s+\w+\s*;/i, 'DELETE without WHERE clause deletes all rows'],
  ['UPDATE without WHERE', /UPDATE\s+\w+\s+SET\s+[^;]+(?<!\s+WHERE\s+[^;]+);/i, 'UPDATE without WHERE clause affects all rows'],
  ['DROP CONSTRAINT without IF EXISTS', /ALTER\s+TABLE\s+\w+\s+DROP\s+CONSTRAINT\s+(?!IF\s+EXISTS)/i, 'Use DROP CONSTRAINT IF EXISTS'],
];
